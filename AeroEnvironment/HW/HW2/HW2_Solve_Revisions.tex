...
\\
\textcolor{red}{\text{For hydrogen, we use atomic } H\text{, not molecular } H_2.} \\
...
\textcolor{red}{We can also compare quantitatively by taking the slope of $\ln(n)$ vs. altitude on
the MSIS plot: $H \approx 6.9\,\mathrm{km}$ below 100 km (close to air), $H \approx 41.1\,\mathrm{km}$
between 200--500 km (between O and He), and $H \approx 217.7\,\mathrm{km}$ above 600 km (near H, but
lower). The mismatch is mostly from assuming constant $T=300\,\mathrm{K}$ -- MSIS temperatures reach
$\sim\!1000\,\mathrm{K}$ higher up, which raises the true scale height.}
...
\subsection*{(iii.)}
\hspace*{2em}The dominant atmospheric constituent would be O, atomic oxygen, \sout{for both solar
maximum and solar minimum.}

\textcolor{red}{This is the case for solar maximum. At solar minimum, we instead find Helium (He)
to be the dominant constituent.}
...
\textcolor{red}{We must first subtract the spacecraft's own orbital velocity from the threshold:
$v_{th} - v_{orbit} \approx 16.654 - 7.559 \approx 9 \, \mathrm{km/s}$. Integrating the
Maxwell-Boltzmann distribution above that (halved for direction) gives a fraction of about
$2.23\times10^{-34}$ -- essentially zero. Sputtering is negligible.}
