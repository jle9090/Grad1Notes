...
for i = 1:length(TOW_s_PRN5)
    [AZ_PRN5(i),EL_PRN5(i),RANGE_PRN5(i)] = compute_azelrange(ecef_NIST, X_PRN5(i,:));
end

figure()
plot(TOW_s_PRN5, AZ_PRN5, 'LineWidth', 1.5)
xlabel('TOW (S)')
ylabel('Azimuth (deg)')
...
figure()
plot(TOW_s_PRN5, EL_PRN5, 'LineWidth', 1.5)
xlabel('TOW (S)')
...
figure()
plot(TOW_s_PRN5, RANGE_PRN5, 'LineWidth', 1.5)
xlabel('TOW (S)')
ylabel('Range (m)')
...
for i = 1:length(TOW_s_PRN8)
    [AZ_PRN8(i),EL_PRN8(i),RANGE_PRN8(i)] = compute_azelrange(ecef_NIST, X_PRN8(i,:));
end

figure()
plot(TOW_s_PRN8, AZ_PRN8, 'LineWidth', 1.5)
xlabel('TOW (S)')
ylabel('Azimuth (deg)')
...
figure()
plot(TOW_s_PRN8, EL_PRN8, 'LineWidth', 1.5)
xlabel('TOW (S)')
ylabel('Elevation (deg)')
...
figure()
plot(TOW_s_PRN8, RANGE_PRN8, 'LineWidth', 1.5)
xlabel('TOW (S)')
ylabel('Range (m)')
...
% Part 4:
prn_5 = linspace(5,5,length(AZ_PRN5_visible));
plotAzEl(AZ_PRN5_visible, EL_PRN5_visible, prn_5)
hold on
prn_8 = linspace(8,8,length(AZ_PRN8_visible));
plotAzEl(AZ_PRN8_visible, EL_PRN8_visible, prn_8)